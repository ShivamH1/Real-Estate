...
(function () {
  ...
  document.querySelectorAll('a[href^="#"]').forEach(function (link) {
    link.addEventListener("click", function (e) {
      ...
      // If mobile drawer is open, close it first to enable scrolling (starts Lenis)
  ...
  if ("IntersectionObserver" in window && spied.length) {
    ...
    spied.forEach(function (sec) {
      spy.observe(sec);
    });
  }

  /* ---------------- Scroll reveals (.fx / .img-reveal) ---------------- */
  var fxEls = Array.prototype.slice.call(
    document.querySelectorAll(".fx, .img-reveal"),
  );
  ...
  if (form) {
    ...
    function validateField(input) {
      ...
      if (ok && value && input.type === "tel")
        ok = /^(\+91[\s-]?)?[6-9]\d{4}[\s-]?\d{5}$/.test(value);
    ...
    form.querySelectorAll("input, textarea, select").forEach(function (input) {
      ...
      input.addEventListener("input", function () {
        if (fieldWrap(input).classList.contains("invalid"))
          validateField(input);
      });
